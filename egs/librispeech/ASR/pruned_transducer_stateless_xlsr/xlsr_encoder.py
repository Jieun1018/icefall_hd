...
def download_xlsr(model_url='https://dl.fbaipublicfiles.com/fairseq/wav2vec/xlsr2_300m.pt', dir_path='./models'):
    ...
    with FileLock(model_path + ".lock"):
        if not os.path.exists(model_path):
            ...
            logging.info(f"xlsr model downloaded {model_path}")
        else:
            logging.info(f"xlsr model {model_path} already exists.")

    return model_path


if __name__ == '__main__':
    xlsr = XLSREncoder(input_size=1024, w2v_url='ww', output_size=1024)
    inputs = torch.randn([10, 160000])
    length = torch.tensor([160000, 150000, 140000, 130000, 120000, 110000, 100000, 100000, 100000, 10000])
    outputs = xlsr(inputs, length)
