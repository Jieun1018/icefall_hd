...
def greedy_search_multilingual(
    model: Transducer,
    encoder_out: torch.Tensor,
    max_sym_per_frame: int,
    ...
    blank_id = model.decoder[lid].blank_id
    context_size = model.decoder[lid].context_size
    unk_id = getattr(model, "unk_id", blank_id)
    ...
    decoder_out = model.decoder[lid](decoder_input, need_pad=False)
    decoder_out = model.joiner[lid].decoder_proj(decoder_out)

    encoder_out = model.joiner[lid].encoder_proj(encoder_out)
...
def greedy_search_batch_multilingual(
    model: Transducer,
    encoder_out: torch.Tensor,
    encoder_out_lens: torch.Tensor,
    max_lid: torch.Tensor,
    return_timestamps: bool = False,
) -> Union[List[List[int]], DecodingResults]:
    """Greedy search in batch mode. It hardcodes --max-sym-per-frame=1.
    Args:
