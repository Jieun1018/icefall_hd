...
def modified_beam_search(
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        for hyps in A:
          for hyp in hyps:
            if count_tmp==0:
              new_encoder_out = hyp.xs.unsqueeze(0)
            else:
              new_encoder_out = torch.cat((new_encoder_out, hyp.xs.unsqueeze(0)), dim=0)
            count_tmp = 1

        decoder_out = model.decoder(decoder_input, new_encoder_out, need_pad=False).unsqueeze(1)
        decoder_out = model.joiner.decoder_proj(decoder_out)
