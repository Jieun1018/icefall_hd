...
if [ -z "$cv_spk_list" ]; then
  ...
  cat $src_data/spk2utt | awk '{ print $1; }' | local/shuffle_list.pl --srand $seed >$tmp/speakers
...
local/subset_data_dir.sh --spk-list $tmp/speakers_trn $src_data $trn_data
local/subset_data_dir.sh --spk-list $tmp/speakers_cv $src_data $cv_data