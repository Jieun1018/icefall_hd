...
if [[ $spk_list ]]; then
  ...
  local/spk2utt_to_utt2spk.pl < $destdir/spk2utt > $destdir/utt2spk || exit 1;
elif [[ $utt_list ]]; then
  ...
  local/shuffle_list.pl < $srcdir/spk2utt |
    awk -v numutt=$numutt '{ if (tot < numutt){ print; } tot += (NF-1); }' |
    sort > $destdir/spk2utt
  local/spk2utt_to_utt2spk.pl < $destdir/spk2utt > $destdir/utt2spk
elif $perspk; then
  ...
  if $shortest; then
    ...
    if [ -f $srcdir/utt2num_frames ]; then
      ln -sf $(local/make_absolute.sh $srcdir)/utt2num_frames $destdir/tmp.len
    else
